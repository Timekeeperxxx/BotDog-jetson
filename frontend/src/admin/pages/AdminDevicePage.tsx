  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-4">
        <QuickFact icon={<ServerCrash size={16} />} label="项目版本" value="当前版本未接入" />
        <QuickFact icon={<Cpu size={16} />} label="主机资源" value="当前版本未接入" />
        <QuickFact icon={<HardDrive size={16} />} label="存储资源" value="当前版本未接入" />
        <QuickFact icon={<Network size={16} />} label="网络连通" value={`${data.networkInterfaces.length} 个已登记网口`} />
      </div>

      <AdminCard
        title="主机信息"
        subtitle="当前项目已有 /api/v1/system-info，只读取静态配置。"
                ...
                <tbody>
                  {data.networkInterfaces.map((item) => (
                    <tr key={item.iface_id}>
                      ...
                      <TableCell className="text-xs text-zinc-500">当前版本未接入</TableCell>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <AdminCard title="危险操作区" subtitle="必须二次确认；当前没有对应后端接口的操作全部保持禁用。">
        <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-4">
          {dangerActions.map((action) => (
            <div key={action.key} className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
              ...
              <div className="mt-4 text-sm font-medium text-white">{action.title}</div>
              <div className="mt-2 text-sm text-zinc-300">{action.description}</div>
              <div className="mt-2 text-xs text-zinc-500">当前版本未启用</div>
              <div className="mt-4">

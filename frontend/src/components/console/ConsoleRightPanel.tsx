...
export function ConsoleRightPanel({
  ...
  return (
    <aside className="h-full min-h-0 w-64 shrink-0 overflow-hidden bg-black flex flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.5)]">
      <LogPanel
        logs={logs}
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        >
          <span className="text-[9px] font-black uppercase tracking-widest text-white/70">统计数据</span>
          {isAiStatsExpanded ? <ChevronDown size={12} className="text-white/60" /> : <ChevronUp size={12} className="text-white/60" />}
        </div>
        {isAiStatsExpanded && (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-zinc-900/20">
          {alerts.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-3">
              <ShieldCheck size={40} className="text-white/20" />
              <p className="text-[10px] uppercase font-black tracking-widest text-white/60">监测运行中...</p>

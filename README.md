...
状态可设为 `keep`，需要重新显示则设为 `on`。冷启动时流水线默认给相机 30 秒
静默初始化时间，之后确认 RTSP 视频轨真实可用才开始转码；失败探测会指数退避。
连续 3 次探测不到视频轨时，看门狗会调用相机自带的维护脚本重启图像服务，并验证
进程、554 监听端口及真实视频轨；仍无视频时自动升级为相机整机重启。恢复期间不重启
Jetson，也不需要重新插拔摄像头电源；两轮自动恢复至少间隔 5 分钟。
USB 摄像头晚于服务启动枚举时，看门狗会持续发现并自动接入。

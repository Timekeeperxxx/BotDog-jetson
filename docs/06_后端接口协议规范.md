...
* ​**URL、方法**​: `GET /api/v1/system/startup`
* ​**权限要求**​: 无需 JWT (公开探针)
* ​**功能说明**​: 返回最近一次启动过程的摘要、生成时间与快照文件路径，便于现场排障与交付验收。
* ​**请求参数**​: 无
* ​**响应体结构**​:
  * `status` (String): 启动结果状态，如 `ok`、`degraded`、`failed`
  * `uptime` (Float): 服务运行秒数
  * `generated_at` (String): 摘要生成时间
  * `snapshot_file` (String): 启动摘要快照文件路径
  * `items` (Array): 启动项数组，每项包含 `name`、`status`、`message`
* ​**示例响应**​:
  ```
  {
    "status": "ok",
    "uptime": 12.8,
    "generated_at": "2026-05-13T10:20:30+08:00",
    "snapshot_file": "data/snapshots/startup_summary.json",
    "items": [
      { "name": "database", "status": "ok", "message": "initialized" }
    ]
  }
  ```
...
控制链路使用 Z2-Mini 的 GCU 私有协议（TCP `2332`）。状态查询要求查看权限，
所有写操作要求操作员权限。

| 方法与路径 | 请求体 | 说明 |
|------------|--------|------|
| `GET /api/v1/gimbal/status` | 无 | 查询连接状态、相对/绝对角度、模式、变焦、OSD 与设备版本 |
| `POST /api/v1/gimbal/mode` | `{"mode":"head_follow"}` | 切换 `angle`、`head_lock`、`head_follow` 或 `fpv` 模式 |
| `POST /api/v1/gimbal/center` | 无 | 切到跟随模式并回中 |
| `POST /api/v1/gimbal/position` | `{"pitch_deg":-30,"yaw_deg":20}` | 设置相对俯仰角和偏航角 |
| `POST /api/v1/gimbal/jog` | `{"pitch_velocity_dps":0,"yaw_velocity_dps":12}` | 短时点动，后端按配置自动发送停止命令 |
| `POST /api/v1/gimbal/zoom` | `{"action":"in"}` | `in`、`out` 或 `stop`，放大/缩小同样带自动停止 |
| `POST /api/v1/gimbal/picture-mode` | `{"mode":"thermal"}` | 切换纯可见光、纯热成像或两种主/小窗画中画模式 |
| `POST /api/v1/gimbal/settings` | `{"osd_enabled":false}` | 开关相机原生 OSD |

实机固件没有可靠执行补光、夜视、对焦和数字变焦切换指令，因此这些选项不对外
提供写接口；状态响应中仍保留设备上报的只读状态。角度限制为俯仰
`-90°..30°`、偏航 `-170°..170°`，点动速度最大 `20°/s`。后端启动时默认
切换到纯可见光模式。
...
后端对 `HTTPException` 使用统一错误响应封装，响应体包含：

- `detail`: 原始错误详情，兼容历史客户端
- `message`: 归一化后的可读消息
- `status_code`: HTTP 状态码
- `error`: 机器可读错误码，如 `unauthorized`、`forbidden`、`not_found`、`bad_request`、`internal_error`
...
* Web 手动控制通过 `POST /api/v1/control/command`、`POST /api/v1/control/stop`、`POST /api/v1/control/e-stop` 和 `POST /api/v1/control/e-stop/reset` 下发。
* 后端不接收早期设想的 `/ws/control` 控制 WebSocket 指令。
* 控制指令必须经过 `ControlArbiter`、急停状态、连接状态和限流检查；FT24/遥控器接管优先级高于 Web 手动控制。
* Z2-Mini 点动与镜头连续变焦由后端独立自动停止，不依赖浏览器补发停止指令。

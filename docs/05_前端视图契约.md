...
以当前 React 主控台（`frontend/src/IndustrialConsoleComplete.tsx`）为基线，说明各视图区域依赖的后端接口、WebSocket 消息和前端事件契约。
...
React 实现由 `useFullscreenControl` 和视频舞台组件维护全屏状态，HUD 叠层在全屏状态下仍保留关键姿态信息。
...
* 启动摘要入口：排障时可按需读取 `/api/v1/system/startup`。

数据来源：

* HTTP `GET /api/v1/system/health` 周期性轮询或由 WebSocket 状态事件更新。
* HTTP `GET /api/v1/system/startup` 按需查询，用于启动摘要展示或调试。

## 8. 交互事件契约 (前端 -> 后端)

### 8.1 控制指令 (摇杆/键鼠)

当前 Web 控制通过 HTTP 接口发送，不再使用早期设想的 `/ws/control` 控制通道。

| 操作 | 接口 |
|------|------|
| 运动指令 | `POST /api/v1/control/command` |
| 停止 | `POST /api/v1/control/stop` |
| 急停 | `POST /api/v1/control/e-stop` |
| 解除急停 | `POST /api/v1/control/e-stop/reset` |

所有运动指令都要经过后端控制权仲裁、急停状态和安全监督检查；FT24/遥控器接管优先级高于 Web 手动控制。
